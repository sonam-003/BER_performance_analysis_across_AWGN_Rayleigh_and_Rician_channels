...
title('BER Performance over Rician Channel (K=4)');
xlabel('SNR (dB)');
...
for mod = 1:length(modulations)
    ...
    for idx = 1:length(SNR_dB)
        ...
        h = sqrt(K_lin/(K_lin+1)) + sqrt(1/(2*(K_lin+1))) * ...
        ...
        switch M
            case 2
                ber_theory(idx) = 0.5 * erfc(sqrt((K_lin*snr_linear)/(K_lin + snr_linear + 1)));
            case 4
                ber_theory(idx) = 0.5 * erfc(sqrt((K_lin*snr_linear)/(2*(K_lin + snr_linear + 1))));
            case 16
                ber_theory(idx) = 3/8 * erfc(sqrt((K_lin*snr_linear)/(10*(K_lin + snr_linear + 1))));
            case 64
                ber_theory(idx) = 7/24 * erfc(sqrt((K_lin*snr_linear)/(42*(K_lin + snr_linear + 1))));
        end
    end
